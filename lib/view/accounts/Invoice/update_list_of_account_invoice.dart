              ...
              scrollDirection: Axis.horizontal,
                child: ResponsiveDataTableWidget(
                  ...
                  columnConfigs: [
                    ...
                    TableColumnConfig(title: "TOTA", sizeType: ColumnSizeType.medium),
                  ...
                  rowBuilder: (item, widths) {
                    ...
                    if (item is Map && item['type'] == 'ADMIN_FEES') {
                      return [
                        ...
                        "", "", "", "", "", "",
                        Text("£${controller.updateInvoiceByIdModel?.accountInvoice?.accountInvoice?.account?.adminFees ?? "0"}", style: mozillaTextSemiBoldText(fontWeight: FontWeight.w900)),
                        "",
                      ];
                    }
                    ...
                    return [
                      Center(child: Text(booking?.referenceNumber ?? "-")),
                      Center(child: Text("${booking?.pickupDate ?? ""} ${booking?.pickupTime ?? ""}")),
                      Center(child: Text((booking?.pickup ?? "-").toUpperCase())),
                      Center(child: Text((booking?.dropoff ?? "-").toUpperCase())),
                      Center(child: Text((booking?.name ?? "-").toUpperCase())),
                      Center(child: Text((booking?.vehicleType?.name ?? "-").toUpperCase())),
                      Center(child: Text((booking?.journeyType?.journeyType ?? "-").toUpperCase())),
                      Center(child: Text((booking?.paymentType?.name ?? "-").toUpperCase())),
