          ...
          pw.Row(
            ...
            children: [
              pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  pw.Text('NAME: ${mainBooking.name ?? ''}', style: const pw.TextStyle(fontSize: 10)),
                  pw.Text('EMAIL: ${mainBooking.email ?? ''}', style: const pw.TextStyle(fontSize: 10)),
                  pw.Text('MOBILE: ${mainBooking.mobile ?? ''}', style: const pw.TextStyle(fontSize: 10)),
                  pw.Text('TELEPHONE: ${mainBooking.telephone ?? ''}', style: const pw.TextStyle(fontSize: 10)),
                ],
              ),
              pw.Column(
                crossAxisAlignment: pw.CrossAxisAlignment.start,
                children: [
                  ...
                  pw.Text('STATUS: ${(mainBooking.bookingStatus?.bookingStatus ?? '').toUpperCase()}',
                      style: const pw.TextStyle(fontSize: 10)),
                  pw.Text('DATETIME: $dateTimeStr', style: const pw.TextStyle(fontSize: 10)),
                  if (returnBooking != null)
                    pw.Text('RETURN DATETIME: $returnDateTimeStr', style: const pw.TextStyle(fontSize: 10)),
                ],
              ),
            ],
          ),
          ...
          pw.Container(
            ...
            child: pw.Row(
              children: [
                ...
                pw.Expanded(
                  child: pw.Text(
                    '£ ${mainBooking.totalCharges ?? mainBooking.fares ?? '0.00'}',
                    style: pw.TextStyle(
                      fontSize: 11,
                      fontWeight: pw.FontWeight.bold,
                    ),
...
pw.Widget _pdfRow(String label1, String val1, String label2, String val2) {
  return pw.Padding(
    ...
    child: pw.Row(
      children: [
        pw.Expanded(
          child: pw.Text('$label1 $val1'.trim(), style: const pw.TextStyle(fontSize: 10)),
        ),
        pw.SizedBox(width: 10),
        if (label2.isNotEmpty)
          pw.Expanded(
            child: pw.Text('$label2 $val2'.trim(), style: const pw.TextStyle(fontSize: 10)),
          ),
      ],
    ),
  );
