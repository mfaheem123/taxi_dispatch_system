...
// (Optional) format nicely
      ...
      if (tempStoreMils == null) {
        ...
        // FIXED: Now safely subtracts one-way AND via miles to get the true return miles.
        final double total = double.tryParse(totalDistance.value) ?? 0.0;
        final double oneWay = double.tryParse(tempStoreMils ?? '') ?? 0.0;
        final double via = double.tryParse(tempStoreViaMils ?? '') ?? 0.0;

        tempStoreReturnMils = (total - oneWay - via).toString();
      }
      ...
      final storedTemFare = await getFares(
        ...
        returnMiles: dropOffTwoWayController.text.isNotEmpty && dropOffTwoWayController.text.isNotEmpty? (double.parse(totalDistance.value)-double.parse(postMils.toString())).toString() : null,
      );
